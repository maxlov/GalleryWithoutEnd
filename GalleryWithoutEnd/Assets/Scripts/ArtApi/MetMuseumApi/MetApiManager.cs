...
{
    public class MetApiManager : BaseApiManager
    {
        ...
        private const string baseApiUrl = "https://collectionapi.metmuseum.org/public/collection/v1";
        ...
        IEnumerator SearchDataRequest()
        {
            ...
            if (searchData.objectIDs.Count == 0 || searchData == null)
            {
            ...
            {
                int objectId = searchData.objectIDs[Random.Range(0, searchData.objectIDs.Count)];
                artDataUrl = baseApiUrl + "/objects/" + objectId;
                ...
                if (data.isPublicDomain)
                {
                    Debug.Log($"Public domain art found in {loopCount} queries.");
                    break;
                }
